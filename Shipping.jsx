...
function Shipping() {
  ...
  const validateForm = () => {
    ...
    const emailRegex = /^[^\s@]+@gmail\.com$/; // must end with @gmail.com
    ...
    if (!cityRegex.test(form.city)) return "City can only contain letters.";
    if (!countryRegex.test(form.country)) return "Country can only contain letters.";
    return null;
  };
  ...
  const handleChange = (e) => {
    ...
    if (name === "postalCode") {
      // Allow only numbers
      const onlyNums = value.replace(/[^0-9]/g, "");
      setForm((prev) => ({ ...prev, postalCode: onlyNums }));
      return;
    }
  ...
  const handleDelete = async (id) => {
    const res = await fetch(`http://localhost:5000/addresses/${id}`, { method: "DELETE" });
    if (res.ok) {
      loadAddresses();

...
import Order from '../../models/Supplier/Order.js';
import Supplier from '../../models/Supplier/Supplier.js';
import Sequence from '../../models/Supplier/Sequence.js';
...
export const updateOrder = async (req, res) => {
  try {
    const { fruit, quantity, supplier: supplierId, deliveryDate, status, trackingStatus, paymentStatus } = req.body;
    ...
    if (Object.keys(updateData).length > 0) {
      if (paymentStatus === 'Paid') {
        console.log(
          `Notification: Payment Success for order ${updated.orderId}.`
        );
      } else {
        console.log(

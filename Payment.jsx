...
const Payment = () => {
  ...
  const validate = () => {
    ...
    // Remove spaces before checking 16 digits
  ...
  const handleChange = (e) => {
    ...
    // ✅ Cardholder Name: allow only letters & spaces
    if (name === 'name') {
      inputValue = inputValue.replace(/[^A-Za-z ]/g, ''); // remove numbers & symbols
    }

    // ✅ Card number formatting (XXXX XXXX XXXX XXXX)
    if (name === 'cardNumber') {
      inputValue = inputValue.replace(/\D/g, ''); // remove non-digits
      inputValue = inputValue.replace(/(.{4})/g, '$1 ').trim(); // add space every 4 digits
      if (inputValue.length > 19) inputValue = inputValue.slice(0, 19); // limit to 16 digits + 3 spaces
    }

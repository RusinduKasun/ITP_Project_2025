...
import { 
  ...
  FaChevronLeft, FaChevronRight, FaDownload, FaWallet, FaPercentage
} from 'react-icons/fa';
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'finance') fetchFinanceSummary();
  }, [activeTab]);
  ...
  const fetchFinanceSummary = async () => {
    ...
    try {
      ...
      // endpoint returns an object with { summary, charts, tables, insights }
      const data = res.data && res.data.summary ? res.data.summary : res.data;
  ...
  const downloadUsersPDF = (onlySelected = false) => {
    ...
    generateStyledPDF({
      title: onlySelected ? 'Selected Users Report' : 'User Report',
      columns: tableColumn,
      rows: tableRows,
      fileName: onlySelected ? 'selected_user_report.pdf' : 'user_report.pdf',
      summary: [`Total Records: ${tableRows.length}`],
  ...
  const downloadSupplierStatsPDF = () => {
    ...
    const summary = [
      `Total Orders: ${supplierStats.totalOrders || 0}`,
      `Pending Orders: ${supplierStats.pendingOrders || 0}`,
      `Delivered Orders: ${supplierStats.deliveredOrders || 0}`,
      `Total Revenue: Rs. ${(supplierStats.totalRevenue ?? 0).toFixed(2)}`
    ];

    generateStyledPDF({
      title: 'Supplier Stats Report',
      columns: tableColumn,
      rows: tableRows,
      fileName: 'supplier_stats_report.pdf',
      summary,
      primaryColor: [34, 197, 94],
      logoText: 'SFL'
    });
        ...
        <div className="flex items-center justify-between border-b border-secondary-200 pb-3">
          <div className="flex space-x-2" role="tablist" aria-label="Admin sections">
            ...
            <button
              ...
              className={`px-4 py-2 font-medium rounded-t-md focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-400 ${activeTab === 'finance' ? 'bg-white border border-b-0 border-secondary-200 text-primary-700' : 'text-secondary-600 bg-transparent'}`}
  ...
  <div className="bg-white rounded-xl p-6 shadow-md hover:shadow-lg transform hover:-translate-y-1 transition-all duration-150 mb-8">
        ...
        <div className="bg-white rounded-xl p-6 shadow-md hover:shadow-lg transform hover:-translate-y-1 transition-all duration-150 mb-8">
          <h2 className="text-xl font-semibold text-secondary-900 mb-4">Finance Overview</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            <div className="p-4 bg-white rounded-lg shadow-md">
              <p className="text-sm text-secondary-600">Total Income</p>
              <p className="text-2xl font-bold text-secondary-900">{financeLoading ? 'Loading...' : `Rs. ${(financeSummary?.totalIncome ?? 0).toLocaleString()}`}</p>
            </div>
            <div className="p-4 bg-white rounded-lg shadow-md">
              <p className="text-sm text-secondary-600">Total Expenses</p>
              <p className="text-2xl font-bold text-secondary-900">{financeLoading ? 'Loading...' : `Rs. ${(financeSummary?.totalExpenses ?? 0).toLocaleString()}`}</p>
            </div>
            <div className="p-4 bg-white rounded-lg shadow-md">
              <p className="text-sm text-secondary-600">Profit / Loss</p>
              <p className="text-2xl font-bold text-secondary-900">{financeLoading ? 'Loading...' : `Rs. ${(financeSummary?.profit ?? 0).toLocaleString()}`}</p>
            </div>
            <div className="p-4 bg-white rounded-lg shadow-md">
              <p className="text-sm text-secondary-600">Profit Margin</p>
              <p className="text-2xl font-bold text-secondary-900">{financeLoading ? 'Loading...' : `${(financeSummary?.profitMargin ?? 0).toFixed(1)}%`}</p>
            </div>
          </div>
          <FinanceOverview embedded />
        </div>
      ) : (
        <>

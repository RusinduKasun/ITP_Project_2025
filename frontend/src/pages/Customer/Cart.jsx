...
function Cart() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/productOrder")
      .then(res => res.json())
      .then(data => {
        if (data.users) {
  ...
  const removeFromCart = async (itemId) => {
    try {
      await fetch(`http://localhost:5000/api/productOrder/${itemId}`, { method: 'DELETE' });
        ...
        {cartItems.length === 0 ? (
          ...
          <div className="cart-container">
            ...
            <div className="cart-summary">
              ...
              <button className="checkout-btn" onClick={() => navigate('/shipping')}>Proceed to Checkout</button>
            </div>
          </div>
        )}
      </div>

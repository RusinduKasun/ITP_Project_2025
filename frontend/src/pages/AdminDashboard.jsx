...
const AdminDashboard = () => {
  ...
  const downloadSalesPDF = () => {
    if (!salesData.length) return alert("No sales data to download");
  ...
  const salesLabels = salesData.map(item => `${item._id.day}/${item._id.month}/${item._id.year}`);
  ...
  const salesOptions = {
    responsive: true,
    plugins: { legend: { position: 'top' }, title: { display: true, text: 'Daily Sales Chart' } },
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {/* Tab Navigation */}
      <div className="mb-6 flex space-x-2 border-b border-secondary-200">
        <button
          className={`px-4 py-2 font-medium focus:outline-none ${activeTab === 'users' ? 'border-b-2 border-primary-600 text-primary-700' : 'text-secondary-600'}`}
          onClick={() => setActiveTab('users')}
        >
          User Management
        </button>
        <button
          className={`px-4 py-2 font-medium focus:outline-none ${activeTab === 'sales' ? 'border-b-2 border-primary-600 text-primary-700' : 'text-secondary-600'}`}
          onClick={() => setActiveTab('sales')}
        >
          Sales Analytics
        </button>
        <button
          className={`px-4 py-2 font-medium focus:outline-none ${activeTab === 'supplier-stats' ? 'border-b-2 border-primary-600 text-primary-700' : 'text-secondary-600'}`}
      ...
      {/* Download Buttons (placed below tabs) */}
        ...
        {activeTab === 'sales' && (
          <button onClick={downloadSalesPDF} className="bg-primary-500 hover:bg-primary-600 text-white font-medium px-4 py-2 rounded-lg flex items-center space-x-2 shadow-sm">
            <FaDownload /> <span>Download Sales PDF</span>
          </button>
        )}
    ...
    {error && <div role="alert" className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-6">{error}</div>}
      ...
      {activeTab === 'users' && (
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-secondary-100 p-6 flex items-center">
      ...
      {activeTab === 'sales' ? (
        <div className="card p-6 mb-8">
          ...
          {salesLoading ? <p>Loading sales chart...</p> : <Line data={salesChartData} options={salesOptions} />}
        </div>
      ) : activeTab === 'supplier-stats' ? (
        <div className="card p-6 mb-8">

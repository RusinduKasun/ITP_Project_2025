...
import { fetchOrders, fetchSuppliers } from '../../api';
...
// Import other supplier pages so they can be shown inside this dashboard
...
export default function Dashboard() {
  ...
  // active tab for this supplier dashboard: overview | calendar | orders | orderdetails | reports | response | suppliers
  ...
  // Ensure totalPrice is treated as a Number and default to 0 when missing
  const totalRevenue = fetchedOrders.reduce((sum, o) => sum + (Number(o.totalPrice) || 0), 0);

      // ප්‍රතිලාභය වැඩිම පළතුරු (Fruit Demand) ගණනය කිරීම
      // Coerce quantity to Number to avoid string concatenation or NaN
      const fruitDemandMap = fetchedOrders.reduce((acc, o) => {
        const qty = Number(o.quantity) || 0;
        acc[o.fruit] = (acc[o.fruit] || 0) + qty;
        return acc;
      }, {});
      ...
      const ordersByMonth = fetchedOrders.reduce((acc, o) => {
        ...
        if (!isValidDate(d)) return acc; // skip invalid dates
        const month = format(startOfMonth(d), 'MMM yyyy');
      ...
      const notifications = fetchedOrders
        .map(order => {
          ...
          const hoursUntilDelivery = isValidDate(deliveryTime) ? differenceInHours(deliveryTime, new Date()) : null;
          ...
          if (hoursUntilDelivery !== null && hoursUntilDelivery <= 24 && hoursUntilDelivery > 0 && order.status === 'approved') {
            notificationsList.push({
              id: `${order._id}-urgent`,
              type: 'warning',
  ...
  // Only block render for overview tab; other tabs render their own loading/error states
  if (activeTab === 'overview') {
    if (loading) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
          <FontAwesomeIcon icon={faSpinner} spin className="text-primary-600 text-6xl" />
        </div>
      );
    }
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium ${activeTab === tab.id ? 'bg-primary-500 text-white' : 'bg-white border'}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        <div>
            ...
            <>
              ...
              <div className="fsd-dashboard-header mb-8">
                <div className="flex items-center mb-2">
                  <FontAwesomeIcon icon={faGauge} className="text-primary-600 text-2xl mr-3" />
                  <h1 className="fsd-dashboard-title text-3xl font-bold text-primary-600">Seller Dashboard</h1>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {[
                  ...
                  { title: 'Total Cost', value: `Rs. ${(stats.totalRevenue ?? 0).toFixed(2)}`, icon: faDollarSign, color: 'bg-gradient-to-br from-green-100 to-green-200' },
                ].map((card, index) => (
                  <div
                    key={index}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <FontAwesomeIcon icon={card.icon} className="text-primary-600 text-xl lg:text-2xl opacity-60" />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div className="fsd-notifications bg-white rounded-lg shadow-sm p-4 lg:p-6">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="fsd-notifications-title text-lg font-semibold text-gray-800">Notifications</h3>
                      <button
                      onClick={loadData}
                      className="text-primary-600 hover:text-primary-700 text-sm lg:text-base transition-colors duration-150"
                  ...
                  <div className="space-y-3 max-h-72 lg:max-h-80 overflow-y-auto">
                    {stats.notifications.length === 0 ? (
                      ...
                      stats.notifications.map(notification => (
                        <div
                          key={notification.id}
                          className={`fsd-notification flex items-start justify-between p-3 rounded-md text-sm ${
                          ...
                          <div className="flex items-start">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-xs text-gray-500">
                                {isValidDate(new Date(notification.timestamp)) ? format(new Date(notification.timestamp), 'MMM dd, HH:mm') : ''}
                              </p>
                            </div>
                          </div>
                          <button
                            onClick={() => openModal(orders.find(o => o._id === notification.orderId))}
                            className="text-primary-600 hover:text-primary-700 transition-colors duration-150 ml-2 flex-shrink-0"
                      ...
                      <tbody>
                        {orders.slice(0, 5).map(order => (
                          <tr key={order._id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td className="p-3">{isValidDate(new Date(order.deliveryDate)) ? format(new Date(order.deliveryDate), 'MMM dd, yyyy') : 'N/A'}</td>
                            <td className="p-3">
                              <span
                ...
                {/* Order Details Modal - remains shared; not duplicated */}
                <Modal
                  isOpen={modalIsOpen}
                  ...
                  {selectedOrder && (
                    <>
                      ...
                      <div className="fsd-modal-content space-y-3 text-gray-700 text-sm">
                        ...
                        <p><strong>Delivery Date:</strong> {isValidDate(new Date(selectedOrder.deliveryDate)) ? format(new Date(selectedOrder.deliveryDate), 'MMM dd, yyyy') : 'N/A'}</p>
                        <p><strong>Total Price:</strong> Rs. {(selectedOrder.totalPrice ?? 0).toFixed(2)}</p>
          ...
          {activeTab === 'response' && <SupplierResponse />}

...
export default function Report() {
  ...
  const getMonthlyOrderSummary = () => {
    ...
    return Object.entries(ordersByMonth).map(([month, stats]) => ({
      ...
      avgOrderCost: stats.count ? ((stats.cost ?? 0) / stats.count).toFixed(2) : '0.00',
    }));
  };
  ...
  const getCostByFruitSupplier = (type = 'fruit') => {
    ...
    return Object.entries(costMap).map(([name, stats]) => ({
      ...
      cost: (stats.cost ?? 0).toFixed(2),
      avgPrice: stats.quantity ? ((stats.cost ?? 0) / stats.quantity).toFixed(2) : '0.00',
    }));
  };
  ...
  const getSupplierReliability = () => {
    return suppliers.map(s => {
      ...
      return {
        supplier: s.name,
        totalOrders: total,
  approvalRate: total ? ((approved / total) * 100).toFixed(1) : '0.0',
        deniedOrders: denied,
        avgResponseTime,
  ...
  const getSupplierDeliveryPerformance = () => {
    return suppliers.map(s => {
      ...
      return {
        supplier: s.name,
        totalOrders: total,
        onTimeRate: total ? ((onTime / total) * 100).toFixed(1) : '0.0',
  ...
  const lastMonth = monthlyData.length > 0 ? (monthlyData[monthlyData.length - 1]?.[1] || 0) : 0;
  const forecast = ((lastMonth * 1.1) || 0).toFixed(0); // Simple 10% growth
  return { fruit, total, avgMonthly: (avgMonthly || 0).toFixed(0), forecast };
    });
  };
      ...
      data,
      {
        columnStyles: {
          ...
          4: { cellWidth: 35, halign: 'center', fontStyle: 'bold', 
               textColor: [244, 67, 54] } // Red for high priority
        }
      }
    );
  };
  ...
  <FontAwesomeIcon icon={faSpinner} spin className="text-primary-600 text-4xl" />
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gray-100">
        <div className="container mx-auto px-6 py-8 max-w-7xl">
          <div className="flex items-center mb-8">
            <FontAwesomeIcon icon={faChartPie} className="text-primary-600 text-2xl mr-3" />
            <h1 className="text-3xl font-bold text-primary-600">Reports</h1>
          </div>
            ...
            {[
              ...
              <div key={report.key} className="bg-white rounded-lg shadow-sm border p-6">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-semibold text-primary-600">{report.title}</h2>
                  <div className="flex gap-4">
                    ...
                    <button
                      ...
                      className="bg-primary-500 text-white px-4 py-2 rounded hover:bg-primary-700 flex items-center transition-colors duration-200"
                ...
                <div
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${
                    expandedReports[report.key] ? 'max-h-[1000px]' : 'max-h-0'
                  }`}
                >

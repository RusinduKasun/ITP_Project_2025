...
const navItems = [
  { to: '/suppliersDash', label: 'Dashboard', icon: faGauge, description: 'Overview & Analytics' },
            ...
            {navItems.map((item, index) => {
              ...
              return (
                <li key={item.to} className="group">
                  <Link
                    to={item.to}
                    className={`relative flex flex-col items-center px-6 py-3 rounded-xl transition-all duration-300 transform hover:scale-105 ${isActive
                  ...
                  >
                    <div className={`flex items-center space-x-3 ${isActive ? 'animate-pulse' : ''}`}>
                      <FontAwesomeIcon
                        icon={item.icon}
                        className={`text-xl transition-colors duration-300 ${isActive ? 'text-emerald-200' : 'text-gray-400 group-hover:text-emerald-400'
                          }`}
                      />
